...
int GRASP(vector < vector <int> > &x, vector < int > &y, vector < int > &z, vector < int > &demanda, vector < vector < float > > &dist, set <int> C, vector <int> &sedes, int Cap, int qM, int dMin, int nC, int R, float alpha, int GRASPmax)
{
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < (int)arguments.size(); i += 2)
    {
        ...
        if (arg_name == "--alpha" )
        {
            alpha = stof(arg_value);
        }
        else if (arg_name == "--GRASPmax")
        {
            GRASPmax = stoi(arg_value);
        }
    }

    ifstream dados(argv[1]);
    dados >>  Cap;
    dados >> qM;
    dados >> R;
    dados >> dMin;
...
cout << -GRASP(x, y, z, demanda, dist, C, sedes, Cap, qM, dMin, nC, R, alpha, GRASPmax);
